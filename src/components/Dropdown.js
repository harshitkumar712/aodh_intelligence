...
if(!selection.some(current=>current.id===item.id)){
 ...
 const handleItemSelection=(item)=>{
    ...
    >
    <Wrapper>
			<Container  onClick={()=>toggle(!open)} onKeyPress={()=>toggle(!open)}>
	...
	<div>{<i className={open?'fas fa-times':"fas fa-chevron-down"}></i>}</div>
			...
			{open&&(
				<Content>
					{items.map(item=>(
						<li key={item.id}>
							<Item onClick={()=>handleClick(item)}><span>{item.value}</span><span>{<i className={handleItemSelection(item)?'fas fa-check-square':'far fa-square'}></i>}</span></Item>
						</li>
						))}
				</Content>
				)
